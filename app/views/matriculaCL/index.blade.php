    	...
    	<div id="example1_wrapper" class="dataTables_wrapper form-inline" role="grid">
        	<div class="row">
        		...
        		<div class="col-xs-6">
        			<div id="example1_filter" class="dataTables_filter">
                        {{ HTML::link('matriculascl/add.html','Agregar Matricula') }}
        			</div>
        		</div>
        	</div>
            <table aria-describedby="example1_info" id="example1" class="table table-bordered table-striped dataTable">
                ...
                <tbody aria-relevant="all" aria-live="polite" role="alert">
                	@foreach( $matriculascl as $matricula)
                	<tr class="odd">
                            <td class=" "><b>{{ $matricula->id }}</b></td>
                            <td class=" "><b>{{ $matricula->codAlumno }}</b></td>
                            <td class=" ">{{ $matricula->codCargaAcademica_cl }}</td>
                            <td class=" ">
                            {{ HTML::link('matriculascl/edit/'.$matricula->id,'Modificar') }}
                            {{ HTML::link('matriculascl/delete/'.$matricula->id,'Eliminar') }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
